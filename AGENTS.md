...
3. The runtime library supports iOS 13 and Mac Catalyst 13 or later. It does not support macOS because `PageRoute` depends on UIKit. The macOS platform in `RouteKitMacros/Package.swift` applies only to the compiler-plugin host.
...
5. Keep macro declarations in `RouteKitMacros/Sources/RouteKitMacro` and compiler-plugin implementations in `RouteKitMacros/Sources/RouteKitMacros`.
6. After changing `RouteKitMacros/Sources/RouteKitMacros`, run `./build.sh` and commit the refreshed `Prebuilt/RouteKitMacros` used by CocoaPods. Keep this binary tracked through Git LFS as configured in `.gitattributes`; never commit it as a regular Git blob.
...
8. Changes to matching, registration, priority, interceptors, middleware, or route execution must include corresponding tests in `Tests/RouteKitTests`. Macro behavior changes must include tests in `RouteKitMacros/Tests/RouteKitMacrosTests`.
9. Validate SwiftPM with a generic iOS device build and format xcodebuild output with xcbeautify. Run the iOS test suite on an available simulator and run `swift test` from `RouteKitMacros` for macro expansion tests.
